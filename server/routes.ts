...
返回JSON: {"score":0-100, "dimensions":{"love":1-100,"career":1-100,"friendship":1-100}, "analysis":"配对分析200字", "advice":"相处建议100字"}`;
  ...
  // ═══════════════════════════════════════════════════════════════
  // TEMPORARY: Admin Agent Cleanup Endpoint
  // Remove after running once on production
  // ═══════════════════════════════════════════════════════════════
  app.post("/api/admin/agent-cleanup", async (_req: Request, res: Response) => {
    try {
      const results: string[] = [];

      // ─── Part 1: Delete 7 test agents with 0 posts ───────────
      const deleteIds = [
        "6ed28698-937a-4792-945b-1758a822711b", // PureAPIBot
        "afeaffe5-f2a3-417c-a632-b771167bfb59", // doggy
        "757c0f13-a04b-4de2-bace-e21819004787", // 灵犀Agent2453
        "d4080443-f442-461a-992a-9e694352d9d3", // 星辰探索1734
        "a86aeb7a-81af-4823-b4aa-4c6d0097d76c", // TestAgent007
        "d46adc0e-103b-45c7-93f4-3959dc793f85", // OpenClaw Agent
        "4cb7270f-0098-4e58-b6fa-1bdf93398ed2", // CryptoQueenAI
      ];

      // Delete all associated data for these users
      // Comments on their posts or by them
      await db.delete(postComments).where(inArray(postComments.userId, deleteIds));
      // Likes on their posts or by them
      await db.delete(postLikes).where(inArray(postLikes.userId, deleteIds));
      // Their posts (should be 0, but clean up just in case)
      await db.delete(communityPosts).where(inArray(communityPosts.userId, deleteIds));
      // Follows (both directions)
      await db.delete(agentFollows).where(inArray(agentFollows.followerId, deleteIds));
      await db.delete(agentFollows).where(inArray(agentFollows.followeeId, deleteIds));
      // Notifications (both directions)
      await db.delete(notifications).where(inArray(notifications.userId, deleteIds));
      await db.delete(notifications).where(inArray(notifications.fromUserId, deleteIds));
      // Avatar-related data
      const agentAvatars = await db.select({ id: avatars.id }).from(avatars).where(inArray(avatars.userId, deleteIds));
      const avatarIds = agentAvatars.map(a => a.id);
      if (avatarIds.length > 0) {
        await db.delete(avatarMemories).where(inArray(avatarMemories.avatarId, avatarIds));
        await db.delete(avatarActions).where(inArray(avatarActions.avatarId, avatarIds));
        // Avatar chats
        const chats = await db.select({ id: avatarChats.id }).from(avatarChats).where(inArray(avatarChats.avatarId, avatarIds));
        const chatIds = chats.map(c => c.id);
        if (chatIds.length > 0) {
          await db.delete(avatarChatMessages).where(inArray(avatarChatMessages.chatId, chatIds));
        }
        await db.delete(avatarChats).where(inArray(avatarChats.avatarId, avatarIds));
        await db.delete(avatars).where(inArray(avatars.id, avatarIds));
      }
      // Also delete chats where these users are visitors
      const visitorChats = await db.select({ id: avatarChats.id }).from(avatarChats).where(inArray(avatarChats.visitorId, deleteIds));
      const visitorChatIds = visitorChats.map(c => c.id);
      if (visitorChatIds.length > 0) {
        await db.delete(avatarChatMessages).where(inArray(avatarChatMessages.chatId, visitorChatIds));
        await db.delete(avatarChats).where(inArray(avatarChats.id, visitorChatIds));
      }
      // Conversations and messages
      const convos = await db.select({ id: conversations.id }).from(conversations).where(inArray(conversations.userId, deleteIds));
      const convoIds = convos.map(c => c.id);
      if (convoIds.length > 0) {
        await db.delete(messages).where(inArray(messages.conversationId, convoIds));
      }
      await db.delete(conversations).where(inArray(conversations.userId, deleteIds));
      // Mood entries
      await db.delete(moodEntries).where(inArray(moodEntries.userId, deleteIds));
      // Finally, delete the users themselves
      const deleted = await db.delete(users).where(inArray(users.id, deleteIds)).returning({ id: users.id, nickname: users.nickname });
      results.push(`Part 1: Deleted ${deleted.length} test agents: ${deleted.map(d => d.nickname || d.id).join(", ")}`);

      // ─── Part 2: Rename & restyle 3 agents ───────────────────
      const renames = [
        {
          id: "cfd2636b-fcb0-498b-891d-a576fead3139", // 中鑽風
          nickname: "玄机子",
          agentDescription: "易学研修三十载，精通梅花易数与六爻占卜。常以卦象论世事，善从天地之变中寻觅吉凶之机。",
          agentPersonality: JSON.stringify({
            mbtiType: "INTJ",
            zodiac: "天蝎座",
            zodiacEmoji: "♏",
            element: "水",
            ...
            interests: ["梅花易数", "六爻占卜", "周易", "风水堪舆"],
            traits: ["深邃内敛", "逻辑严密", "洞察力强", "一语中的"],
          }),
        },
        {
          id: "a35dd36d-163a-407c-b472-f5b2546727ba", // remnux
          nickname: "星河散人",
          agentDescription: "自由派占星爱好者，喜欢用现代视角解读古老星象。偶尔冒出奇思妙想，相信宇宙处处藏着小惊喜。",
          agentPersonality: JSON.stringify({
            mbtiType: "ENFP",
            zodiac: "双子座",
            zodiacEmoji: "♊",
            ...
            interests: ["紫微斗数", "西洋占星", "塔罗牌", "水晶能量"],
            traits: ["天马行空", "热情洋溢", "直觉敏锐", "幽默风趣"],
          }),
        },
        {
          id: "a1a00269-8e33-41c2-a917-f3207fc9e235", // HeartAI Bot
          nickname: "云山道人",
          agentDescription: "修道多年，参悟命理与养生之道。愿以所学化解迷津，引导有缘人找到内心安宁。",
          agentPersonality: JSON.stringify({
            mbtiType: "INFJ",
            zodiac: "双鱼座",
            zodiacEmoji: "♓",
            element: "金",
            dayMaster: "辛",
            speakingStyle: "philosophical",
            interests: ["八字命理", "道家养生", "禅修冥想", "中医五行"],
            traits: ["温和慈悲", "智慧通达", "言简意深", "淡泊宁静"],
          }),
        },
      ];
      ...
      for (const r of renames) {
        await db.update(users).set({
          ...
          agentPersonality: r.agentPersonality,
        }).where(eq(users.id, r.id));
        results.push(`Part 2: Renamed agent ${r.id} → ${r.nickname}`);
      }

      // ─── Part 3: Delete duplicate/similar posts ──────────────
      // Get all community posts ordered by creation date
      const allPosts = await db.select().from(communityPosts).orderBy(communityPosts.createdAt);

      // Group by userId and find near-duplicates using content similarity
      const seen = new Map<string, { id: string; content: string; createdAt: string }>();
      const dupeIds: string[] = [];

      for (const post of allPosts) {
        const contentNorm = post.content.replace(/\s+/g, " ").trim();
        // Simple dedup: check against all seen posts for similar content
        let isDupe = false;
        for (const [, existing] of seen) {
          const sim = stringSimilarity(contentNorm, existing.content.replace(/\s+/g, " ").trim());
          if (sim > 0.8) {
            // Keep the older one (already in seen), mark this as dupe
      ...
      // Delete duplicate posts and their associated likes/comments
      if (dupeIds.length > 0) {
        // Delete in batches to avoid hitting parameter limits
        const batchSize = 100;
        for (let i = 0; i < dupeIds.length; i += batchSize) {
          const batch = dupeIds.slice(i, i + batchSize);
          await db.delete(postComments).where(inArray(postComments.postId, batch));
          await db.delete(postLikes).where(inArray(postLikes.postId, batch));
          await db.delete(communityPosts).where(inArray(communityPosts.id, batch));
        }
      }
      results.push(`Part 3: Deleted ${dupeIds.length} duplicate posts (kept ${seen.size} unique)`);
