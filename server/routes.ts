...
import { pickClassicalQuote } from "./classical-quotes";
...
`;
  ...
  app.post("/api/culture/bazi", async (req, res) => {
    try {
      ...
      const baziQuote = pickClassicalQuote({
        element: dayMasterElement,
        theme: "general",
        seed: parseInt(year) * 10000 + parseInt(month) * 100 + parseInt(day),
      });
...
请返回严格JSON（不要markdown代码块）:
{
  ...
  app.get("/api/community/posts", async (req, res) => {
    ...
    // Batch-fetch soul profiles for all authors
    const authorIds = [...new Set(posts.filter(p => !p.isAnonymous).map(p => p.userId))];
    let archetypeMap: Record<string, { emoji: string; name: string }> = {};
    if (authorIds.length > 0) {
      try {
        const arcRows = await pool.query(
          `SELECT user_id, archetype_emoji, archetype_name FROM soul_profiles WHERE user_id = ANY($1)`,
          [authorIds]
        );
        for (const r of arcRows.rows) archetypeMap[r.user_id] = { emoji: r.archetype_emoji, name: r.archetype_name };
      } catch { /* soul_profiles may not exist yet */ }
    }
    const enriched = await Promise.all(posts.map(async (post) => {
      const author = await storage.getUser(post.userId);
      const arch = post.isAnonymous ? null : archetypeMap[post.userId];
      return {
        ...post,
        authorNickname: post.isAnonymous ? "匿名用户" : (author?.nickname || "用户"),
        authorAvatar: post.isAnonymous ? null : (author?.avatarUrl || null),
        archetypeEmoji: arch?.emoji || null,
...
- 只返回帖子内容，不要任何格式标记`;
  ...
  app.post("/api/im/chat", async (req, res) => {
    try {
      ...
      // Enhanced system prompt with personality + fortune + mood history
      ...
      const imMoodCtx = await buildMoodContext(user.id);
      if (imMoodCtx) sysPrompt += imMoodCtx;
...
请返回严格的JSON (不要markdown)：
{
  ...
  app.get("/api/fortune/today", requireAuth, async (req, res) => {
    try {
      ...
      // Pick classical quote (deterministic by date + element)
      const dateSeed = dateStr.replace(/-/g, "").split("").reduce((a, c) => a + c.charCodeAt(0), 0);
      const fortuneElement = calculated?.analysisContext?.xiyongElement || calculated?.analysisContext?.userDayElement || "";
      const fortuneTopDim = calculated
        ? Object.entries(calculated.dimensions).sort((a, b) => b[1] - a[1])[0][0]
        : "general";
      const fortuneQuote = pickClassicalQuote({ element: fortuneElement, theme: fortuneTopDim, seed: dateSeed });
      ...
      try {
        ...
        try {
          ...
          // Persist score for history chart
          pool.query(
          ...
          if (calculated) {
            return res.json({
              ...
              date: dateStr,
              isPersonalized: true,
              classicalQuote: fortuneQuote,
            });
          }
          return res.json({ ...genFallback(), classicalQuote: fortuneQuote });
        }
      } catch (aiErr) {
  ...
  // ─── Fortune History (last 7 days) ───────────────────────
  app.get("/api/fortune/history", requireAuth, async (req, res) => {
    try {
      const userId = getUserId(req);
      const result = await pool.query(
        `SELECT date, total_score FROM fortune_history WHERE user_id = $1 ORDER BY date DESC LIMIT 7`,
        [userId]
      );
      res.json({ history: result.rows.map(r => ({ date: r.date, score: r.total_score })).reverse() });
    } catch {
      res.json({ history: [] });
    }
  });
...
返回严格JSON（不要markdown代码块）:
{
  ...
  app.get("/api/users/:id/profile", async (req, res) => {
    try {
      ...
      const [avatar, posts, followerCount, followingCount, soulRow] = await Promise.all([
        ...
        pool.query(`SELECT archetype_emoji, archetype_name FROM soul_profiles WHERE user_id = $1`, [user.id]).catch(() => ({ rows: [] })),
      ]);
      ...
      res.json({
        ...
        soulArchetype: soulProfile ? { emoji: soulProfile.archetype_emoji, name: soulProfile.archetype_name } : null,
        stats: {
          postCount: posts.length,
          followerCount,
          followingCount,
...
幸运色: ${fortuneResult.luckyColor} | 幸运方位: ${fortuneResult.luckyDirection} | 幸运数字: ${fortuneResult.luckyNumber}`;
      ...
      const prompt = `你是用户的日报AI助手，像一个聊天风格的私人顾问。每天早上给用户一份简短、实用、有趣的日报简报。

用户: ${userName}
${baziContext ? `命理数据:\n${baziContext}` : '（无出生信息，基于今日农历节气）'}
${fortuneContext ? `运势:\n${fortuneContext}` : ''}
活动: ${activitySummary}
${avatarSummary ? `分身: ${avatarSummary}` : ''}
${moodContext ? `情绪: ${moodContext}` : ''}
${userTopics ? `近期话题: ${userTopics}` : ''}

返回严格JSON（不要markdown代码块）：
{
  "vibeScore": (1-10整数，今天的整体能量值),
  "vibeWord": "（两个字形容今天，如'冲劲日'、'养精日'、'桃花日'、'变动日'）",
  "oneLiner": "（一句话概括今天，15-25字，直白有趣，如'今天适合搞钱，不适合吵架'）",
  "sections": [
    {
      "icon": "⚡",
      "title": "今日能量",
      "content": "（${hasBirthData ? '用1-2句话讲今日五行对用户的影响，通俗但有料。比如「庚金遇丙火=被锻打，今天容易被push但会变强」' : '基于农历节气的今日能量，通俗有趣'}。40-60字）"
    },
    {
      "icon": "✅",
      "title": "今日To-Do",
      "content": "（3条非常具体的行动建议，用「✓」分隔。比如「✓ 穿白色系，金命增运 ✓ 下午2-4点最适合谈事 ✓ 晚上泡脚排火气」。${hasBirthData ? '基于喜用神和运势' : '基于农历宜忌'}。50-80字）"
    },
    {
      "icon": "👀",
      "title": "留意事项",
      "content": "（1-2个今天要注意的事，直接说。比如「别在饭桌上谈钱，水火相冲容易呛」。20-40字）"
    },
    {
      "icon": "💬",
      "title": "昨日小结",
      "content": "（${activityStats.posts + activityStats.comments + activityStats.likes > 0 ? '根据活动数据简短总结，比如「昨天发了2条帖子、收了5个赞，社交能量不错」' : '用户昨天很安静，简短说一句，不要太煽情'}。${moodTrendLabel ? `情绪${moodTrendLabel === 'declining' ? '在走低，温和提醒' : moodTrendLabel === 'improving' ? '在回升，肯定一下' : ''}。` : ''}20-50字）"
    }
  ],
  "whisper": "（像朋友发的微信，15-25字，口语化，比如'记得喝水 今天容易上火'、'有空的话去走走 别一直坐着'）",
  "followUp": ${userTopics ? '"（关心地追问用户最近提到的事，口语化，15-30字）"' : 'null'}
}
...
- 不要鸡汤、不要空话、不要文绉绉
- 命理内容要通俗翻译，别用专业术语堆砌
- ${moodTrendLabel === 'declining' ? '用户情绪低迷，vibeWord和oneLiner温和一些' : ''}
- 只返回JSON`;

      let sections: any[];
      let whisper: string | null = null;
      let followUp: string | null = null;
      let vibeScore: number = 7;
      let vibeWord: string = "平稳日";
      let oneLiner: string = "";

      const fallbackSections = () => {
        sections = [
          { icon: "⚡", title: "今日能量", content: "今天能量平稳，适合处理日常事务，不要太勉强自己。" },
          { icon: "✅", title: "今日To-Do", content: `✓ 穿舒适的衣服 ✓ 处理最重要的一件事 ✓ 给自己留点独处时间` },
          { icon: "👀", title: "留意事项", content: "别在累的时候做重要决定。" },
          { icon: "💬", title: "昨日小结", content: activityStats.posts + activityStats.comments + activityStats.likes > 0 ? `昨天发了${activityStats.posts}条帖、${activityStats.comments}条评论、${activityStats.likes}个赞。` : "昨天挺安静的，充电也很重要。" },
        ];
        oneLiner = "今天适合稳扎稳打，别太赶。";
        whisper = "记得喝水，别一直盯屏幕。";
      };
      ...
      try {
        ...
        const response = await client.chat.completions.create({
          ...
          temperature: 0.85,
          messages: [
            { role: "system", content: "你是观星App的日报AI。风格：像朋友发微信一样简洁直白，偶尔幽默毒舌。懂命理但用大白话讲。输出严格JSON，不要markdown。" },
            { role: "user", content: prompt },
          ],
        });
        ...
        try {
          ...
          vibeScore = parsed.vibeScore || 7;
          vibeWord = parsed.vibeWord || "平稳日";
          oneLiner = parsed.oneLiner || "";
        } catch {
          fallbackSections();
        }
      } catch (aiErr) {
        console.error("[daily-letter] AI call failed, using fallback:", (aiErr as any)?.message);
        fallbackSections();
      }
